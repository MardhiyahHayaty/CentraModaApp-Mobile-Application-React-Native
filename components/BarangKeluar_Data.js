...
export default function BarangKeluar({navigation}) {
  ...
  const getBarangKeluar = async () => {
    try {
      const response = await fetch(
        "https://7ef1-114-125-22-202.ap.ngrok.io/api/barang_keluars"
    ...
    return(
      ...
      <View style = {{
        ...
        <View style={styles.isiBarang}>
          ...
          <Text style={{fontSize:12, fontWeight:'400', color:'#101452', marginTop: 8}}>{props.tgl_keluar}</Text>
          <Text style={{fontSize:12, fontWeight:'normal', color:BiruTua, marginTop:7.6, width: 200}}>Pegawai {props.nama_pegawai}</Text>
          </View>
